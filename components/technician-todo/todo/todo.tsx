  ...
  return (
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      <header className="mb-8">
        ...
        <Badge className="text-sm text-white mr-4">
          {tasks && tasks.technicianName}
        </Badge>
        <Badge variant="default" className="text-sm text-white ">
          {tasks && formatDate(tasks.inspectionDate.toString() ?? "")}
        </Badge>
      </header>
      <main>
        <ul className="space-y-6">
          {tasks &&
            tasks.cars?.map((car) => (
              <li
                key={car.inspectionScheduleId}
                className="border border-gray-300 p-4 rounded-lg hover:cursor-pointer"
              >
                <div
                  className="flex items-start justify-between mb-2"
                  onClick={() => setIsCarDetailsOpen(true)}
                >
                  <div>
                    <h2 className="font-semibold">
                      {car.modelName}-{car.licensePlate}
                    </h2>
                    <p className="text-sm text-gray-600">
                      {car.inspectionAddress}
                    </p>
                  </div>
